...
class Meeting {
    ...
    constructor(daySelected, titre, descriptions) {
        ...
        //creation de la div qui contiendera le button de sumission
        ...
        // ajouter le description du rendez vous
        ...
        //desactivation les input et le button de soumission
        if (this.description != "" && this.titre != "") {

            // si le rendez-vous existe, on desactive le champ tittre, description et le boutton soumettre
...
class Calendar {
    ...
    loadMonth(date) {
        ...
        //function cette fonction prend en parametres une date, elle verifie si une date est feriée ou pas
        function JoursFeries(dateferies) {
            var joursFerie = "aucun";
            //liste des jours feries avec le mois dans l'année
            var arrayJoursferies = new Array("1/1", "1/5", "08/5", "14/7", "15/8", "1/11", "11/11", "25/12");
        ...
        for (let i = 1; i <= monthLength + 1; i++) {
            ...
            let dayMark = document.createElement('span');
            var nbRendezVous = 0;
            ...
            var cellToday = this.today.getDate() + "/" + this.today.getMonth();
            var cellSelect = newDate.getDate() + "/" + newDate.getMonth();
            //style css des weekend
            if (newDate.getDay() == 0 || newDate.getDay() == 6) {
                if (((newDate.getDay() - 1) != this.today.getDay()) && cellToday != cellSelect) {

                    cell.style.color = "red";
                    cell.style.background = "#183133";
                }
            }


            //ajout le marqueur pour le jour fernier
            if (JoursFeries(cheekJourFeries) != "aucun") {
                dayMark.textContent = ".";
                this.content.appendChild(cell).appendChild(dayMark);
    ...
    meeting(daySelectedAuto) {
        ...
        if (typeof (Storage) !== "undefined") {
            while (CountRendezVous > 0 && CountRendezVous < 7) {
                ...
                if (saveMeeting == null) {
                    ...
                    let meetingSaved = new Meeting(daySelectedAuto, saveMeeting.titre, saveMeeting.description);
